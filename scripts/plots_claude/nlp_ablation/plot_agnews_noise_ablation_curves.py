...
CLI:
    python -m scripts.plots_claude.nlp_ablation.plot_agnews_noise_ablation_curves \
        --runs-root results \
        --out-dir results/plots_claude/nlp_ablation
...
from scripts.plots_claude._common import walk_runs
...
def _collect(
    ...
    per_key: dict[str, list[tuple[int, pathlib.Path, dict]]] = {
        spec.key: [(seed, p, r) for seed, (p, r) in sorted(by_key_seed[spec.key].items())]
        for spec in PLOT_LINES
    }
...
def _summary_lines(
    ...
    for st in stats:
        ...
        lines.append(f"    mean_peak_val_acc_pct: {st.mean_peak_acc * 100.0:.2f}")
        lines.append(f"    mean_final_val_acc_pct: {st.mean_final_acc * 100.0:.2f}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary_path = write_summary(
        args.out_dir, NAME, _summary_lines(args.runs_root, stats, pdf_path, png_path)
    )
