...
CLI:
    python -m scripts.plots_claude.nlp_ablation.plot_agnews_noise_ablation_barplot \
        --runs-root results \
...
from scripts.plots_claude.nlp_ablation.plot_agnews_noise_ablation_curves import (
    PLOT_LINES,
    N_EXPECTED_SEEDS,
    LineSpec,
    _aggregate,
...
def _build_figure(stats):
    ...
    peak_stds = np.array([st.std[int(np.argmax(st.mean))] * 100.0 for st in stats])
